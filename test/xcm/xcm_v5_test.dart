...
void _xcmPalletSend(MetadataApi metadata, {int loop = 5}) {
  ...
  test(
    ...
    () {
      ...
      final fromJson = XCMCallPalletTransferAssetsUsingTypeAndThen.fromJson(
        decodeCall.data,
      );
      expect(fromJson.serializeVariant(), encode.sublist(1));
      expect(fromJson.toJson(), decodeCall.data);
  ...
  test(
    ...
    () {
      ...
      final fromJson = XCMCallPalletLimitedTeleportAssets.fromJson(
        decodeCall.data,
      );
      expect(fromJson.serializeVariant(), encode.sublist(1));
      expect(fromJson.toJson(), decodeCall.data);
  ...
  test(
    ...
    () {
      ...
      final fromJson = XCMCallPalletReserveTransferAssets.fromJson(
        decodeCall.data,
      );
      expect(fromJson.serializeVariant(), encode.sublist(1));
      expect(fromJson.toJson(), decodeCall.data);
  ...
  test(
    ...
    () {
      ...
      final fromJson = XCMCallPalletLimitedReserveTransferAssets.fromJson(
        decodeCall.data,
      );
      expect(fromJson.serializeVariant(), encode.sublist(1));
      expect(fromJson.toJson(), decodeCall.data);
...
class _XCMTestGenerate {
      ...
      default:
        ...
        return XCMV5FungibilityNonFungible(
          instance: switch (type) {
            XCMAssetInstanceType.array8 => XCMV5AssetInstanceArray8(
              datum: generateRandom(8),
            ),
            XCMAssetInstanceType.array4 => XCMV5AssetInstanceArray4(
              datum: generateRandom(4),
            ),
            XCMAssetInstanceType.array16 => XCMV5AssetInstanceArray16(
              datum: generateRandom(16),
            ),
            XCMAssetInstanceType.array32 => XCMV5AssetInstanceArray32(
              datum: generateRandom(32),
            ),
            XCMAssetInstanceType.indexId => XCMV5AssetInstanceIndex(
              index: BigInt.from(100),
            ),
  ...
  XCMV5Asset createAsset() {
    return XCMV5Asset(
      id: XCMV5AssetId(location: createLocation()),
      fun: createFung(),
    );
  }
  ...
  XCMTransferType createTransferType() {
    final type = XCMTransferTypeType.values.elementAt(
      _generateRandInt(XCMTransferTypeType.values.length),
    );
    switch (type) {
  ...
  XCMV5AssetFilter createFilterAsset() {
    return XCMV5AssetFilterWild(
      asset: XCMV5WildAssetAllOf(
        id: createAsset().id,
        fun: XCMV5WildFungibilityFungible(),
      ),
    );
  }
  ...
  XCMV5ExpectError createError() {
    ...
    switch (type) {
      ...
      case XCMV5ErrorType.trap:
        return XCMV5ExpectError(
          error: XCMV5ErrorTrap(code: BigInt.from(1)),
          index: 1,
        );
  ...
  XCMV3MaybeErrorCode createMybeErrorCode() {
    final type = XCMV3MaybeErrorCodeType.values.elementAt(
      _generateRandInt(XCMV3MaybeErrorCodeType.values.length),
    );
    switch (type) {
  ...
  XCMInstructionV5 createInstruction(XCMInstructionType type) {
    return switch (type) {
      XCMInstructionType.withdrawAsset => XCMV5WithdrawAsset(
        assets: createAssets(),
      ),
      XCMInstructionType.reserveAssetDeposited => XCMV5ReserveAssetDeposited(
        assets: createAssets(),
      ),
      XCMInstructionType.receiveTeleportedAsset => XCMV5ReceiveTeleportedAsset(
        assets: createAssets(),
      ),
      ...
      XCMInstructionType.hrmpChannelAccepted => XCMV5HrmpChannelAccepted(
        recipient: 2,
      ),
      XCMInstructionType.hrmpChannelClosing => XCMV5HrmpChannelClosing(
        sender: 1,
        initiator: 2,
        recipient: 3,
      ),
      XCMInstructionType.clearOrigin => XCMV5ClearOrigin(),
      XCMInstructionType.descendOrigin => XCMV5DescendOrigin(
      ...
      XCMInstructionType.setErrorHandler => XCMV5SetErrorHandler(
        xcm: createXCM(notIn: [type]),
      ),
      XCMInstructionType.setAppendix => XCMV5SetAppendix(
        xcm: createXCM(notIn: [type]),
      ),
      XCMInstructionType.clearError => XCMV5ClearError(),
      XCMInstructionType.claimAsset => XCMV5ClaimAsset(
        assets: createAssets(),
      ...
      XCMInstructionType.expectAsset => XCMV5ExpectAsset(
        assets: createAssets(),
      ),
      XCMInstructionType.expectOrigin => XCMV5ExpectOrigin(
        location: createLocation(),
      ),
      XCMInstructionType.expectError => createError(),
      XCMInstructionType.expectTransactStatus => XCMV5ExpectTransactStatus(
      ...
      XCMInstructionType.lockAsset => XCMV5LockAsset(
        asset: createAsset(),
        unlocker: createLocation(),
      ),
      XCMInstructionType.unlockAsset => XCMV5UnlockAsset(
        asset: createAsset(),
        target: createLocation(),
      ),
      XCMInstructionType.noteUnlockable => XCMV5NoteUnlockable(
        asset: createAsset(),
        owner: createLocation(),
      ),
      XCMInstructionType.requestUnlock => XCMV5RequestUnlock(
        asset: createAsset(),
      ...
      XCMInstructionType.aliasOrigin => XCMV5AliasOrigin(
        origin: createLocation(),
      ),
      XCMInstructionType.unpaidExecution => XCMV5UnpaidExecution(
        weightLimit: createWeightLimit(),
      ),
      XCMInstructionType.payFees => XCMV5PayFees(asset: createAsset()),
      XCMInstructionType.initiateTransfer => XCMV5InitiateTransfer(
        destination: createLocation(),
        remoteFees: XCMV5AssetTransferReserveDeposit(
          asset: createFilterAsset(),
        ),
  ...
  XCMV5 createXCM({
    ...
    while (types.length < 3) {
      final type = XCMInstructionType.values.elementAt(
        _generateRandInt(XCMInstructionType.values.length),
      );
