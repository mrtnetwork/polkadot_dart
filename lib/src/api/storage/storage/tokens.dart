...
enum SubstrateStorageTokensMethods {
  totalIssuance("TotalIssuance"),
  accounts("Accounts");

  final String method;
  const SubstrateStorageTokensMethods(this.method);
}

/// safexcmversion
class SubstrateStorageTokens extends SubstrateStorageApi {
  const SubstrateStorageTokens();
  @override
  SubstrateStorageApis get api => SubstrateStorageApis.tokens;
  Future<List<QueryStorageFullResponse<BigInt>>> totalIssuanceEntries({
    ...
    final locations = api.getStreamStorageEntries(
      ...
      >(
        ...
        methodName: SubstrateStorageTokensMethods.totalIssuance.name,
    ...
    final locations = api.getStreamStorageEntries(
      request:
          GetStreamStorageEntriesRequest<QueryStorageFullResponse<T>, Object>(
            palletNameOrIndex: this.api.name,
            inputs: [addressBytes],
            methodName: SubstrateStorageTokensMethods.accounts.name,
