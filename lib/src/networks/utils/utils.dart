...
class SubstrateNetworkControllerUtils {
  ...
  static bool locationEquality({
    ...
    if (a.isZeroParents() && b.isOneParents()) {
      ...
      if (CompareUtils.iterableIsEqual(
        ...
        b.interior.junctions.sublist(1),
      )) {
        return true;
      }
      ...
      if (CompareUtils.iterableIsEqual(
        b.interior.junctions,
        a.interior.junctions.sublist(1),
      )) {
        return true;
      }
    }
  ...
  static XCMJunction createVersionedAccountJunction({
    ...
    return switch (address.type) {
      SubstrateAddressType.substrate => XCMJunctionAccountId32.fromVersion(
        keyBytes: toBytes,
        version: version,
      ),
      SubstrateAddressType.ethereum => XCMJunctionAccountKey20.fromVersion(
        keyBytes: toBytes,
        version: version,
      ),
    };
  ...
  static XCMMultiLocation createBeneficiaryLocation({
    ...
    final junactions = XCMJunctions.fromVersion(
      junctions: [account],
      version: version,
    );
  ...
  static XCMMultiLocation createDestination({
    ...
    if (destinationLocation.isEqual(originlocation)) {
      return XCMMultiLocation.fromVersion(
        parents: 0,
        interior: XCMJunctions.fromVersion(
          junctions: [if (account != null) account],
          version: version,
        ),
        version: version,
      );
  ...
  static XCMVersionedLocation locationWithGeneralKey({
    ...
    if (key == null) {
      final locationIndex = List<int>.filled(
      ...
      junctions.add(
        XCMJunctionGeneralKey.fromVersion(
          length: 2,
          data: locationIndex,
          version: version,
        ),
  ...
  static XCMVersionedLocation locationWithPalletInstanceAndAccountKey20({
    ...
    List<XCMJunction> junctions = [
      ...
      XCMJunctionPalletInstance.fromVersion(
        index: palletInstance,
        version: version,
      ),
      if (accountKey != null)
        XCMJunctionAccountKey20.fromVersion(
          keyBytes: BytesUtils.fromHexString(accountKey),
          version: version,
        ),
    ];
    return XCMVersionedLocation.fromLocation(
    ...
    return events.where(
      (event) {
        ...
        final deposit = switch (pallet) {
          SubtrateMetadataPallet.assets =>
            SubstrateAssetsDepositedEvent.fromJson(
              event.inputAs<Map<String, dynamic>>(),
            ),
          SubtrateMetadataPallet.tokens =>
            SubstrateTokensDepositedEvent.fromJson(
              event.inputAs<Map<String, dynamic>>(),
            ),
  ...
  static SubstrateXCMTransctionTrackerResult? findProcessedXCMMessage({
    ...
    final List<SubstrateXcmProcessEvent> processedEvents = events.where(
      (event) {
      ...
      pallets: ["XcmpQueue", "MessageQueue"],
      catchError: true,
    );
    if (processedEvents.isEmpty) return null;
    final xcmEvent = processedEvents.firstWhereNullable(
      (e) => StringUtils.hexEqual(e.xcmMessageId, id),
    );
    final deposits = _extractDeposit(events: events, params: params);
    if (xcmEvent != null) {
      return SubstrateXCMTransctionTrackerResult(
        status:
            (xcmEvent.success && deposits.isNotEmpty)
    ...
    try {
      ...
      XCMMultiLocation assetLocation() => getParaReserveLocation(
        params.params.assets[0].getlocalizedLocation(
