...
abstract mixin class SubstrateNetworkControllerAssetQueryHelper {
  ...
  getAssetsPalletAssetIdentifierBigInt(
    ...
    final assetEntries = await SubstrateQuickStorageApi.assets.assetEnteries(
      api: provider.metadata.api,
      rpc: provider.provider,
    );
    return {
  ...
  static Future<Map<BigInt, Map<String, dynamic>?>>
  getAssetsPalletMetadataIdentifierBigInt(
    MetadataWithProvider provider, {
    List<BigInt>? assetIds,
  }) async {
    if (assetIds != null) {
    ...
    final assetEntries = await SubstrateQuickStorageApi.assets.metadataEnteries(
      api: provider.metadata.api,
      rpc: provider.provider,
    );
    return {
  ...
  getFungiblesPalletAssetsIdentifierMultiLocation(
    ...
    final assetEntries = await SubstrateQuickStorageApi.fungibles.assetEnteries(
      api: provider.metadata.api,
      rpc: provider.provider,
    );
    return {
  ...
  getForeignAssetsPalletMetadataIdentifierMultiLocation(
    MetadataWithProvider provider,
    XCMVersion version, {
    List<XCMVersionedLocation>? locations,
  }) async {
    if (locations != null) {
  ...
  getXcAssetConfigPalletAssetIdToLocationEntriesIdentifierBigInt(
    ...
    return {
      for (final i in locations)
        i.storageKey.inputAsBigInt(0): XCMVersionedLocation.fromJson(
          i.response,
        ).asVersion(version),
    };
  }
  ...
  getAssetRegistryPalletMetadataIdentifierInt(
    MetadataWithProvider provider, {
    List<int>? knownIds,
  }) async {
    if (knownIds != null) {
  ...
  getAssetRegistryPalletCurrencyIdToLocationsEntriesIdentifierMap(
    ...
    return {
      for (final i in assetEntries)
        i.storageKey.inputAsMap(0): XCMMultiLocation.fromJson(
          json: i.response,
          version: version,
        ).asVersioned(version: version),
    };
  ...
  getTokensPalletAccountIdentifierMap({
    required MetadataWithProvider provider,
    required BaseSubstrateAddress address,
    List<Map<String, dynamic>>? assetIds,
  }) async {
    if (assetIds != null) {
