...
typedef ONREQUESTNETWORKCONTROLLER =
    Future<BaseSubstrateNetworkController?> Function(
...
typedef ONREQUESTNETWORKPROVIDER =
    Future<MetadataWithProvider?> Function(BaseSubstrateNetwork from);
...
abstract mixin class SubstrateNetworkControllerXCMTransferBuilder {
  static SubtrateMetadataPallet findXcmPallet(MetadataWithProvider provider) {
    if (provider.metadata.api.palletExists(
  ...
  getAllAvailableXCMTransferMethod(MetadataWithProvider provider) {
    final xcmPallet = findXcmPallet(provider);
    final xcmMethods = provider.metadata.api.getCallMethodNames(xcmPallet.name);
    final xTokenMethod = provider.metadata.api.getCallMethodNames(
      SubtrateMetadataPallet.xTokens.name,
    );
    final xTransfer = provider.metadata.api.getCallMethodNames(
      SubtrateMetadataPallet.xTransfer.name,
    );
    return [
      ...XCMCallPalletMethod.values.where((e) => xcmMethods.contains(e.method)),
      ...XTokenCallPalletMethod.values.where(
        (e) => xTokenMethod.contains(e.method),
      ),
      ...XTransferCallPalletMethod.values.where(
        (e) => xTransfer.contains(e.method),
      ),
    ];
  }
  ...
  static bool transferMethodExists({
    ...
    return switch (method) {
      final XCMCallPalletMethod method => () {
        final xcmPallet = findXcmPallet(provider);
        final xcmMethods = provider.metadata.api.getCallMethodNames(
          xcmPallet.name,
        );
    ...
    if (weight != null) {
      if (weight.version != version) {
        throw DartSubstratePluginException(
          "Invalid weight limit version.",
          details: {"expected": version.type, "version": weight.version.type},
        );
      }
      return weight;
    }
  ...
  static XCMCallPallet transferDotThroughUsingTypeAndThen({
    ...
    List<XCMTransferType> assetsTypes =
        params.assets
            .map(
    ...
    List<(SubstrateXCMTransferAsset, XCMAsset)> assets =
        params.assets
            .map(
              (e) => (
                e,
    ...
    int feeItem = assets.indexWhere(
      (e) => e.$1 == params.assets.elementAt(params.feeIndex),
    );
    ...
    final instruction = assets.map(
      (e) => _createDepositAssets(
    ...
    final XCMVersionedXCM customXcm = XCMVersionedXCM.fromXCM(
      XCM.fromInstructions(
        instructions: [
  ...
  static BaseSubstrateNetwork? _findNetworkFromLocation(
    ...
    return origin.relaySystem.networks.firstWhereNullable(
      (e) => e.paraId == parachain.id,
    );
  }
  ...
  static Future<List<SubstrateTransactionXCMDryRunResult>> _dryRunXcm({
    ...
    BaseSubstrateNetwork? location = _findNetworkFromLocation(
      origin,
      xcmLocation.location,
    );
    if (location == null) {
    ...
    if (!SubstrateQuickRuntimeApi.dryRun.methodExists(
      ...
      return [
        SubstrateTransactionXCMDryRunResult.unsupported(
          location,
    ...
    if (SubstrateQuickRuntimeApi.xcmPayment.methodExists(
      ...
      final feeAssetLocation =
          fees
              .getlocalizedLocation(
                reserveNetwork: location,
                version: location.defaultXcmVersion,
              )
              .location;
  ...
  static XTokenCallPallet _xtokenTransfer({
    ...
    final weightLimit = _createWeight(
      XCMVersion.v3,
      params.destinationWeightLimit,
    );
  ...
  static XTokenCallPallet _xtokenTransferMulticurrencies({
    ...
    List<XTokenTransferTokenWithAmount> transferAssets =
        assets
            .map(
              (e) => XTokenTransferTokenWithAmount(
                amount: e.amount,
                currencyId: e.asset.xTokenTransferIdAs(),
              ),
            )
            .toList();
    final weightLimit = _createWeight(
      ...
      destWeight: switch (params.destinationWeightLimit?.type) {
        ...
        XCMWeightLimitType.limited =>
          _createWeight(
            xTransferXcmVersion,
            params.destinationWeightLimit,
          ).cast<XCMV3WeightLimitLimited>().weight,
      },
    );
  }
  ...
  static List<R> filterReceivedAssets<R extends BaseSubstrateNetworkAsset>({
    ...
    for (final destAsset in assetsWithLocation) {
      ...
      final location = disabledLocations.firstWhereNullable(
        (e) => SubstrateNetworkControllerUtils.locationEquality(
          networkA: network,
          networkB: origin,
          b: dLocation,
          a: e,
  ...
  static XCMDepositReserveAsset _depositReserve({
    ...
    final destination = SubstrateNetworkControllerUtils.createDestination(
      destination: to,
      version: xcmVersion,
      originlocation: reserveLocation,
    );
        ...
        instructions: [
          XCMBuyExecution.fromFees(
            fees: XCMAsset.fungibleAsset(
              amount: amount,
              id: XCMAssetId.fromLocation(feeLocation),
            ),
          ),
          XCMDepositAsset.fromAssets(
            assets: XCMMultiAssetFilter.wild(
  ...
  static Future<XCMCallPallet> transferAssetsThroughUsingTypeAndThen({
    ...
    XCMVersionedXCM createXCM({BigInt? fee, bool canEstimateFee = false}) {
      if (params.hasRelayAsset && !params.destinationNetwork.role.isPara) {
        return XCMVersionedXCM.fromXCM(
          XCM.fromInstructions(
            instructions: [
              _initiateTeleport(
