...
>
    ...
    return XCMMultiLocation.fromVersion(
      ...
      interior: XCMJunctions.fromVersion(
        junctions: [
          ...
          pallet,
          XCMJunctionGeneralIndex.fromVersion(index: assetId, version: version),
        ],
        version: version,
      ),
    ).asVersioned();
  ...
  Future<Map<BigInt, PolkadotAssetHubNetworkAsset>> _getAssets({
    ...
    final pallet = XCMJunctionPalletInstance.fromVersion(
      index: provider.metadata.api.getPalletIndexByName(
        SubtrateMetadataPallet.assets.name,
      ),
      version: version,
    );
    final a =
  ...
  _getForeignAsset({
    ...
    final assetsEntires =
        await SubstrateNetworkControllerAssetQueryHelper.getForeignAssetsPalletAssetIdentifierMultiLocation(
          provider,
          network.defaultXcmVersion,
          assetIds: assetIds,
        );
    final metadatas =
        await SubstrateNetworkControllerAssetQueryHelper.getForeignAssetsPalletMetadataIdentifierMultiLocation(
          provider,
          network.defaultXcmVersion,
          locations: assetIds,
        );
  ...
  getAccountAssetsInternal({
    ...
    List<Object>? knownAssetIds,
    List<BasePolkadotAssetHubNetworkAsset>? knownAssets,
  }) async {
    final provider = await params.loadMetadata(network);
    ...
    if (foreignAssets.isNotEmpty) {
      final balancesEntries =
          await SubstrateNetworkControllerAssetQueryHelper.getForeignAssetsPalletAccountIdentifierMultilocation(
            provider: provider,
            address: address,
...
class PolkadotAssetHubNetworkController
  ...
  Future<SubstrateXCMCallPallet> xcmTransferToParaInternal({
    ...
    final bool isForeignAsset =
        params.assets[0].asset.excutionPallet ==
