...
>
  ...
  Future<Map<Map<String, dynamic>, InterlayAssetMetadata>> _getMetdata({
    ...
    final metadataEntries =
        await SubstrateNetworkControllerAssetQueryHelper.getAssetRegistryPalletMetadataIdentifierInt(
          provider,
          knownIds: ids,
        );
    ...
    for (final i in metadataEntries.entries) {
      ...
      metadata[{
        InterlayAssetType.foreignAsset.type: i.key,
  ...
  getAccountAssetsInternal({
    ...
    List<BaseInterlayNetworkAsset>? knownAssets,
  }) async {
  ...
  getNativeAssetFreeBalance(BaseSubstrateAddress address) async {
    final provider = await params.loadMetadata(network);
    final balance =
        await SubstrateNetworkControllerAssetQueryHelper.getTokensPalletAccountSignleAssetIdentifierMap(
          provider: provider,
          address: address,
