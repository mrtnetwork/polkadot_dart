...
abstract class BaseUniqueworkController<NETWORK extends BaseSubstrateNetwork>
  ...
  getAccountAssetsInternal({
    ...
    List<BigInt>? knownAssetIds,
    List<BaseUniqueNetworkAsset>? knownAssets,
  }) async {
    ...
    if (assets.isNotEmpty) {
      final balancesEntries =
          await SubstrateNetworkControllerAssetQueryHelper.getFungiblePalletBalanceIdentifierBigInt(
            provider: provider,
            address: address,
      ...
      for (final i in balancesEntries.entries) {
        ...
        balances.add(
          SubstrateAccountAssetBalance<UniqueNetworkAsset>(
            asset: asset,
            free: balance,
          ),
        );
