...
class PhalaNetworkController
  ...
  Future<Map<BigInt, PhalaNetworkAsset>> _getAssets({
    ...
    final locations =
        await SubstrateNetworkControllerAssetQueryHelper.getAssetsRegistryPalletIdByLocationsIdentifierMap(
          provider,
          network.defaultXcmVersion,
        );
    final fees =
        await SubstrateNetworkControllerAssetQueryHelper.getAssetsRegistryPalletRegistryInfoByIds(
          provider,
        );
    final a =
        assets.entries.map((e) {
          ...
          final BigInt? executionPrice = MetadataUtils.parseOptional(
            fee?["execution_price"],
          );
  ...
  getAccountAssetsInternal({
    ...
    List<BasePhalaNetworkAsset>? knownAssets,
  }) async {
